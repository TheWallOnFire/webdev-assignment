...
type SubjectFilter = 'all' | 'toan' | 'vat_li' | 'hoa_hoc';

export default function Dashboard() {
  const [students, setStudents] = useState<TopStudent[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [top10Subject, setTop10Subject] = useState<SubjectFilter>('all');
    ...
    const fetchTop = async () => {
      ...
      try {
        ...
        if (axios.isAxiosError(err)) {
          setError(
            err.response?.data?.message || 'Failed to fetch Top 10 students.'
          );
        } else {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="bg-secondary/50 border border-white/5 rounded-xl shadow-sm p-8">
        <div
          className={
            'mb-8 border-b border-white/5 pb-6 flex flex-col sm:flex-row ' +
            'sm:items-center items-start justify-between gap-4'
          }
        >
          <div>
            ...
            <p className="text-sm text-slate-400">
              See the list of top 10 best student
            </p>
          </div>
          <select
            className={
        ...
        <div className="relative min-h-[300px]">
          ...
          {!error && !loading && students.length === 0 && (
            <div
              className={
                'text-center text-slate-500 py-12 flex flex-col items-center'
              }
            >
              <svg
              ...
              >
                <path
                  ...
                  d={
                    ...
                    '01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 ' +
                    '2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 ' +
                    '1 0 006.586 13H4'
                  }
                />
              </svg>
