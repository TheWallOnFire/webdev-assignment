...
const renderActiveShape = (props: any) => {
  ...
  return (
    <g>
      <text x={cx} y={cy - 10} dy={8} textAnchor="middle" fill="#f8fafc" className="font-semibold text-lg">
        {payload.name}
      </text>
      <text x={cx} y={cy + 15} dy={8} textAnchor="middle" fill="#94a3b8" className="text-sm">
...
export function PercentageChart({ chartData }: PercentageChartProps) {
  ...
  return (
    <div className="w-full h-full mt-8">
      <h3 className="text-center text-slate-300 font-medium mb-6">Percentage Breakdown</h3>
      <ResponsiveContainer width="100%" height={340}>
        <PieChart>
          ...
          <Pie
            // @ts-ignore - activeIndex is valid but might not be correctly typed in this recharts version
          ...
          >
            {chartData.map((entry, index) => (
              <Cell 
                ...
                style={{ outline: 'none' }}
                className="transition-all duration-300 hover:brightness-110 cursor-pointer border-2 border-[#0f172a]"
              />
            ))}
          </Pie>
          <Tooltip 
            ...
            formatter={(value: any) => [Number(value).toLocaleString(), 'Students']}
          />
