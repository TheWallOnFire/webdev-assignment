...
const getRankBadgeStyle = (index: number) => {
  switch (index) {
    ...
    case 1: return 'bg-slate-300/20 text-slate-300';
      ...
      <table className="w-full text-sm text-left">
        <thead className="text-xs text-slate-400 uppercase bg-black/20 border-b border-white/5">
          <tr>
            ...
            <th scope="col" className="px-6 py-4 font-medium">SBD</th>
            <th scope="col" className="px-6 py-4 font-medium text-right">Math</th>
            <th scope="col" className="px-6 py-4 font-medium text-right">Physics</th>
            <th scope="col" className="px-6 py-4 font-medium text-right">Chemistry</th>
            <th scope="col" className="px-6 py-4 font-medium text-right text-primary">Total Score</th>
          </tr>
        </thead>
          ...
          {students.map((student, index) => (
            <tr key={student.sbd} className="hover:bg-white/5 transition-colors">
              <td className="px-6 py-4">
                <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${getRankBadgeStyle(index)}`}>
                  {index + 1}
                </span>
              </td>
