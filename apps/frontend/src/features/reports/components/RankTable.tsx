  ...
  switch (index) {
    case 0:
      return (
        ...
        'hover:from-yellow-500/20 border-l-2 border-yellow-500'
      );
    case 1:
      return (
        'bg-gradient-to-r from-slate-300/10 to-transparent ' +
        'hover:from-slate-300/20 border-l-2 border-slate-300'
      );
    case 2:
      return (
        'bg-gradient-to-r from-orange-500/10 to-transparent ' +
        'hover:from-orange-500/20 border-l-2 border-orange-500'
      );
    default:
      ...
      return (
        <span
          className={
            ...
            'bg-white/5 text-slate-400 text-xs font-semibold'
          }
        >
          {index + 1}
        </span>
...
const getScoreStyle = (score: number | string | null, isTotal = false) => {
  ...
  if (isTotal) {
    return (
      ...
      'from-blue-400 to-emerald-400 text-lg'
    );
  }
  ...
  return (
    <div
      className={
        ...
        'bg-[#0f172a]/80 backdrop-blur-md shadow-2xl'
      }
    >
      <table className="w-full text-sm text-left border-collapse">
        <thead
          className="text-xs text-slate-400 uppercase bg-black/40 border-b border-white/10"
        >
          <tr>
            <th
              scope="col"
              className="px-6 py-5 font-semibold tracking-wider w-20 text-center"
            >
              Rank
            </th>
            ...
            {(subjectFilter === 'all' || subjectFilter === 'toan') && (
              <th
                scope="col"
                className="px-6 py-5 font-semibold tracking-wider text-right"
              >
                Math
              </th>
            )}
        ...
        <tbody className="divide-y divide-white/5 text-slate-200">
          {sortedStudents.map((student, index) => (
            <tr
              key={student.id}
              className={`transition-all duration-300 group ${getRowStyle(index)}`}
            >
              <td className="px-6 py-4 text-center">
                <div
                  className={
                    'flex justify-center group-hover:scale-110 ' +
              ...
              {(subjectFilter === 'all' || subjectFilter === 'toan') && (
                <td
                  className={`px-6 py-4 text-right tabular-nums ${getScoreStyle(student.toan)}`}
                >
                  {student.toan}
                </td>
              )}
              {(subjectFilter === 'all' || subjectFilter === 'vat_li') && (
                <td
                  className={`px-6 py-4 text-right tabular-nums ${getScoreStyle(student.vat_li)}`}
                >
                  {student.vat_li}
                </td>
              )}
              {(subjectFilter === 'all' || subjectFilter === 'hoa_hoc') && (
                <td
                  className={
                    `px-6 py-4 text-right tabular-nums ${getScoreStyle(
                      student.hoa_hoc
                    )}`
                  }
                >
              ...
              <td
                className={
                  ...
                  `${getScoreStyle(student.total_score, true)}`
                }
              >
